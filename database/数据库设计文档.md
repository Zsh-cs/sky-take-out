...
| ----------- | ----------- | ------------ | ---------------------------- |
| id          | bigint      | 主键         | 自增                         |
| name        | varchar(32) | 分类名称     | 联合索引(name,deleted)唯一   |
| type        | int         | 分类类型     | 1菜品分类 ，2套餐分类        |
| sort        | int         | 排序字段     | 用于分类数据的排序           |
| status      | int         | 状态         | 1启用，0禁用，默认禁用       |
| create_time | datetime    | 创建时间     |                              |
| create_user | bigint      | 创建人id     |                              |
| update_time | datetime    | 最后修改时间 |                              |
| update_user | bigint      | 最后修改人id |                              |
| deleted     | int         | 逻辑删除字段 | 0未删除，1已删除，默认未删除 |
...
| ----------- | ------------- | ------------ | ---------------------------- |
| id          | bigint        | 主键         | 自增                         |
| name        | varchar(32)   | 菜品名称     | 联合索引(name,deleted)唯一   |
...
| ----------- | ------------- | ------------ | ---------------------------- |
| id          | bigint        | 主键         | 自增                         |
| name        | varchar(32)   | 套餐名称     | 联合索引(name,deleted)唯一   |
| category_id | bigint        | 分类id       | 逻辑外键                     |
...
> [!WARNING]
>
> 不能使用order作为表名，因为与MySQL关键字`order`冲突，会带来一系列不必要的麻烦。
